...
class metafed(torch.nn.Module):
    def __init__(self, args):
        ...
        self.server_model, self.client_model, self.client_weight = modelsel(
            args, args.device)
        self.optimizers = [optim.SGD(params=self.client_model[idx].parameters(), lr=args.lr) for idx in range(args.n_clients)]
    ...
    def update_flag(self, val_loaders):
        for client_idx, model in enumerate(self.client_model):
            _, val_iou,_ = test(
                self.args, model, val_loaders[client_idx], self.loss_fun, self.args.device)
            if val_iou > self.args.threshold:
                self.flagl[client_idx] = True

    def client_train(self,args, c_idx,test_loader, dataloader, round, io):
        ...
        for _ in range(self.args.wk_iters):
            train_loss, train_iou, train_acc = trainwithteacher(
    ...
    def personalization(self, c_idx, dataloader, val_loader):
        ...
        with torch.no_grad():
            _, v1a, _ = test(self.args, model, val_loader, self.loss_fun, self.args.device)
            _, v2a, _ = test(self.args, tmodel, val_loader, self.loss_fun, self.args.device)
